...
GLT.loadmanager.loadFiles({
	...
	"finished" : function(files) {
		var world = files["world.json"]; 
		entities = world.entities; 
		GLT.loadmanager.loadFiles({
			"files" : world.models.concat(world.textures), 
			"update" : function(file, progress) {			
				updateProgress(file, progress); 
			}, 
			"finished" : function(worlddata) {
				for(var file in worlddata) {
					entities[file] = worlddata[file]; 
				}
				setup(files); 
